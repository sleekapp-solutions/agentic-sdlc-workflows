...
FORBIDDEN_PATHS = (".env", ".agents/", ".claude/", ".git/")
...
class ProjectValidator:
    """Runs the target repository's configured validation commands.

    Flutter's analyze/test commands are the safe default inherited from the
    original project. Other repositories can opt in with a semicolon-separated
    ``WORKFLOW_VALIDATION_COMMANDS`` environment value.
    """

    def __init__(self, repo_root: Path, commands: tuple[tuple[str, ...], ...] | None = None) -> None:
        self.repo_root = repo_root
        self.commands = commands or self._configured_commands()

    @staticmethod
    def _configured_commands() -> tuple[tuple[str, ...], ...]:
