...
.spinner { width:16px; height:16px; border:2.5px solid var(--warn-line); border-top-color:var(--warn); border-radius:50%; animation:spin .8s linear infinite; flex:0 0 auto } @keyframes spin { to { transform:rotate(360deg) } }
...
@media (max-width:640px) { .topbar { padding:10px 16px; gap:10px } .repo small { display:none } .repo strong { overflow:hidden; text-overflow:ellipsis; white-space:nowrap } .env summary .chips { order:3; flex-basis:100% } .env-toggle { margin-left:auto } main { padding:18px 16px 60px } .flow-picker,.advanced-grid { grid-template-columns:1fr } .start-row { flex-direction:column; align-items:stretch } #startButton { width:100% } }</style></head><body>
